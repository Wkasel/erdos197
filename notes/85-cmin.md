...
| cmin_t2(16) | local | **= 33 OPTIMAL [255.7 s]** — > 2M; the FIRST of the three assigned exchange scales.  Witness anatomy is a BIGT lesson: t = 2, R = {29, 31}, D = {24, 26} — D kills BOTH SZ′ reps of the evens {36, 38} (w ∈ {12, 16} ⟹ u ∈ {24, 26} = D), so the even team legally owns below-corner evens: at t = ⌊M/8⌋ the confinement hole is real and the adversary uses it — and still pays DOUBLE the target.  Scope of SZ′(t) is exactly right |
| cmin_t2(8), cmin_t2(12) re-runs (encoder x-val) | local | = 10 [0.2 s], = 22 [14.5 s], both OPTIMAL — e188 reproduces notes/71 §11's values from the older instrument; the t2(8) optimum is Bm1-mixed (t₁ = 1: no scope for BM1-VAC at 8, consistent) |
| cmin_t2(20), cmin_t2(24) | sprint-B | [in flight] |
| cmin(24), cmin(28) | sprint-D | **= 24 [87.5 s], = 28 [109.4 s], both OPTIMAL** — pre-registered = M confirmed; witnesses AGAIN the extremal frame: t = 1, (r, d) = (47, 48) resp. (55, 56) = (2M−1, 2M), corner-confined.  The measurement hole between cmin(20) and the M ≥ 32 theorem is CLOSED: **cmin(M) = M at SIX consecutive scales 8..28, and S ≥ M is now known at EVERY M ≡ 0 mod 4 for t = 1** (measured ≤ 28, theorem ≥ 32) |
| cmin(32) | sprint-D | **= 32 OPTIMAL [447.7 s]**, witness (63, 64) = (2M−1, 2M), t = 1 — the SEVENTH consecutive exact scale, and the first where theorem and measurement overlap: CMIN-SMALLT says every t = 1 split at 32 pays ≥ 32, the global optimum IS a t = 1 split paying exactly 32.  **cmin(M) = M at 8/12/16/20/24/28/32, witness = the extremal frame at every scale** |
| cmin(16) re-run (encoder x-val) | local | = 16 OPTIMAL [6.2 s], witness (31, 32) — e188 reproduces the notes/71 e-value with a from-scratch instance |
