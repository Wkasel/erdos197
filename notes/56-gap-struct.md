...
|-----|-----------------------------------------------------|---------|------|
| GAP-DICH | fan-clean ∧ min\|Y\| ≥ K*(M) ⟹ Φ = 0.  UPDATE (notes/57, 2026-08-27 night): the K* law here is CORRECTED to the mechanistic formula K* = m + 9 + max(α_E − f_O, α_O − f_E) (m = M/2; α, f finite catalogue quantities), exact at SIX scales incl. blind predictions at 112/128; the bridge itself is now the proved case tree T/FI/ANCHOR/COLL/MID + H-DICH + SP over the catalogue facts F0–F4 (machine, six scales); residue of the gap = five catalogue-schema sub-gaps (notes/57 §7), all in the GAP-FG species pool | forced-interval calculus + fan-catalogue facts; no structure gap left | medium → low-medium (notes/57) |
...
| GAP-ASM′ | the three thresholds overlap for all M ≡ 0 (16), M ≥ 48.  UPDATE (notes/59 §D): the composition itself is now Theorem ASM′(M) [PROVED], reducing this gap to exactly the inequality (OV-∀): K*(M) ≤ C(M) for all M.  UPDATE 2 (notes/58 §1, 2026-08-27 night): MEASURED at 112/128 — K*(112) = C(112) = 60, K*(128) = C(128) = 68: (OV) holds with W = 0 at both; both offsets went FLAT ((C−1)−b = −5, K*−b = −4 over 96..128); the mod-32 K* reading is dead (see also notes/57 §0.2's mechanistic law); AND the robust chain (DICH-U/DICH-Z(d₀=4)/RP-ARM(4)) is VERIFIED at 128 (notes/58 §4.4), so the fix no longer depends on (OV-∀) at the tested scales | arithmetic of two thresholds; the robust-P-ARM fix is implemented and 128-verified; M = 160 endgame in notes/58 §6 | low-medium (downgraded after 112/128: no hole opened, fix verified) |
