...
parity \cite{DEGS77}. For infinite sets the situation changes completely.
...
it is problem \#197 of the Erd\H{o}s problem collection \cite{Bl}, where it
is stated for $\N$ and carries a Lean formalisation \cite{FC}.

The modern literature around the problem concerns densities and longer
progressions. LeSaulnier and Vijay \cite{LV11} showed that the upper density
of a set admitting an admissible permutation is at least $1/2$ ($\alpha_{\N}(3)
\ge 1/2$) and the lower density at least $1/4$, and conjectured both sharp;
...
The natural candidate for a two-set partition — implicit already in the
parity heuristic of \cite{DEGS77} and adopted as the default attack in all
informal discussion of the problem we are aware of — is the \emph{dyadic
partition}: let
\[
...
(Theorem~\ref{thm:ogred}). Third — and this is the mathematical heart — we
...
possible, and Erd\H{o}s--Graham (1979, p.~338) repeat the question verbatim
...
underlying AP predicate allows zero or negative common differences, but
injectivity of the permutation rules out $d = 0$, and $d < 0$ is the
decreasing case, so the notion of ``monotone $3$-AP'' agrees with ours.)
\end{remark}
...
repository, together with \texttt{reproduce.sh}, a single script that
re-runs the complete verification stack from a pinned environment.
...
structures avoiding three-term arithmetic progressions}, arXiv:2404.13510
(2024).
