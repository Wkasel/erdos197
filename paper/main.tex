...
valid permutation; machine verification gives infeasibility for every
$16 \le M \le 200$ and for $M = 512$, isolating one conjecture as the
remaining content of the negative program for this partition. We also prove a clean
...
\section{The order gadget and the main theorem}\label{sec:og}
...
reduction precisely and proves it unconditionally, reports the machine
verification of its finite instances, and then states honestly what is
proved, what is machine-checked, and what remains open. Throughout,
...
\begin{corollary}[Conditional death of the canonical candidate]
\label{cor:conditional}
If $\OG(M)$ is infeasible for all $M \ge 16$ --- or merely for
infinitely many $M$ of the form $2^{2t-1}$ --- then the canonical
dyadic partition $\Z^+ = \SA \sqcup S_B$ is not a solution of the
Erd\H{o}s--Graham problem: $\SA$ admits no permutation free of monotone
$3$-APs.
\end{corollary}

\subsection{Status of the scale-uniform statement}

Everything above this subsection is unconditional. What is missing is
exactly the hypothesis of Theorem~\ref{thm:ogred}: a proof of
$\OG$-infeasibility at infinitely many dyadic scales. We record the
sharpest machine-checked finite facts, then the conjecture they
support, then the known obstructions to the obvious proof strategies.
...
Theorem~\ref{thm:ogred} needs only the following, which is the
sharpest known form of the missing statement.

\begin{conjecture}[Scale-uniform order-gadget infeasibility]
\label{conj:og}
$\OG(M)$ is infeasible for every $M \ge 16$. Sharpest sufficient form:
for every $M \equiv 0 \pmod 8$, the no-monotone-AP constraint (i) on
$(M, 2M]$ is inconsistent with the three order axioms $\mathrm{C3}$.
\end{conjecture}

\emph{What is known toward Conjecture~\ref{conj:og}, and what
obstructs it.} The full parametric analysis lives in the repository
(notes 27, 28, 30 and the \texttt{e87}--\texttt{e96} experiment
suite); we summarize its verified state. (1) A complete
pencil-checkable refutation of $\OG(40)$ exists (a $27$-node lemma
DAG using only AP-reflection and transitivity), but its root lemma
exploits band collisions unique to $M = 40$ and provably admits no
forced parametric lift; it cannot be the base of a uniform proof.
(2) A layer of forced-order lemmas behind the refutation (bottoms
pushed before their own guards; mid-band collision orders) is
machine-certified at $M \in \{40, 44, 48, 52, 60, 100, 128\}$ --- all
$\equiv 0 \pmod 4$ --- and an adversarial audit shows this layer is
genuinely \emph{parity-locked}: at $M \not\equiv 0 \pmod 4$ several of
its lemmas are forced in the \emph{reverse} orientation, and the
half-integral anchors it uses do not exist, so any uniform proof must
either stay within the residue class $M \equiv 0 \pmod 4$ (which
suffices for the dyadic family) or be rebuilt per residue class.
(3) Deletion-minimal certificate supports grow roughly linearly with
$M$ ($\approx 8$ triples per unit of $M$ between scales $44$ and
$60$), which suggests --- but does not prove --- that no fixed finite
list of parametric AP-triples certifies infeasibility at all scales;
a genuinely scale-uniform mechanism (or a compactness/limit argument
over order types of $(M, 2M]$) appears to be required. (4) The residue
class $M \equiv 3 \pmod{16}$ resists the generic attack prefix and
needs a fourth $16$-attack (verified through $M = 131$); it is
irrelevant to the dyadic family but shows the attack calculus is not
residue-free. These are stated as precise open verifications in the
repository's gap ledger; none of them affects the unconditional
Theorem~\ref{thm:ogred} or the machine record of
Proposition~\ref{prop:ogmachine}.
