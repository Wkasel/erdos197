...
\begin{proof}[Proof strategy for the main theorem (mutual double induction)]
The construction is assembled by induction over pairs (scale, instance type),
where the two instance types are the even-block team $\SA$ and its shifted
copy $S_B$. The phase order within a scale step is: low-valuation phases by
the self-absorbing class orders of Theorem~\ref{thm:vdc}; the
$2\cdot\mathrm{odd}$ phase, which halves onto the odd kernel of an instance of
the \emph{opposite} type at half the scale (supplying the truncation the
class calculus cannot: completions leaving the block land in odd blocks, which
belong to the other team); and the two deferred-class suffixes. The inductive
hypothesis supplies the opposite-type arrangement; the base cases are
machine-checked. \emph{Status: the phase-closure verifications are complete
except for the boundary bookkeeping of the halving step, in progress.}
\end{proof}
