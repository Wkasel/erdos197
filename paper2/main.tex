...
\begin{corollary}[B1 at zero dust --- unconditional]\label{cor:b1zero}
\emph{(H1)} at $C = 0$ is exactly Theorem~\ref{thm:c3p} (the three $C3(p)$
units are among the rung's fired units).  Hence, unconditionally: no
3-permutable set contains infinitely many full dyadic blocks.
\status{PROVED}
\end{corollary}
...
contradiction with Corollary~\ref{cor:b1zero}.}
...
consistency checks \S2.4), notes/80-pincer \S3 (L-NOTAIL); adversarial
machine verification notes/81 \S2 (e186: chart exactness 44/44, AP
transport 44\,400/44\,400, units-in-rung 6/6, fresh rung UNSAT at $p=13$,
witness 4-purity, Geneson-$W$ $\Lambda$-scan 0 hits $t = 5..200$).
Rider-free after notes/86 \S1.}
...
Sharpest surviving special case (all others are closed by
\S\ref{sec:chart}): the cap for gap-$\ge 3$, 2-adically split
(HSPLIT-generic, hence aperiodic) block minorities, of which no inhabitant
...
\begin{verbatim}
 ...
 (Z/D/E(p)/P     (N3-b) [GAP]    |
  L1(p), FLIP(p))                |
          |                    Hyp AFFORD' [GAP]
   B1 at C0=0 [P]              (residue: SPLIT corner;
