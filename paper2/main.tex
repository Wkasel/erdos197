...
internal adversarial prose audit complete, notes/86 \S1 (one display fix: FLIP$(p)$ Case~I
step~3, ``$M > 2p+2$'' $\to$ ``$M > 2p-2$'').  Machine record: e123 schema
$p = 5..25$, e180 \texttt{partMINMsharp} boundary audit (first-pass
scales computed from the scan and asserted equal to the sharp affine
boundaries $M \ge p+7$ for $L1$ and in-class $M \ge 2p+6$ for FLIP, with
every scanned below-threshold scale checked to fail; the legacy
\texttt{partMINM} asserted only the slack bounds $2p+10 / 2p+14$),
solver cross-validation at 9 odd
...
For every $C \ge 0$ there exists $x_0(C)$ such that for every
$p \equiv 1 \pmod 4$ with $3p \ge x_0(C)$ there is $m^*(p, C)$ with
$\Rung(3p, 3p+1; 2^m, D)$ inconsistent whenever $m \ge m^*(p, C)$ and
$D \subseteq B(m)$ with $|D| \le C$.  Under
Hypothesis~\ref{hyp:n3grow} one may take $x_0(C) = \max\{11,\, 4C+5\}$
(then $|D| \le C < \lfloor (3p-1)/4 \rfloor$).
\end{definition}

\begin{remark}[Why not the fixed-$p$ form]\label{rem:h1prime}
The stronger statement ``for every \emph{fixed} $p$ and every $C$''
(the original H1 of notes/52 \S1.4) does \emph{not} follow from
Hypothesis~\ref{hyp:n3grow}, which is robust only below the dust
threshold $\lfloor (x-1)/4 \rfloor$ at $x = 3p$ --- and it is in fact
\emph{machine-refuted} at $p = 5$, $C = 3$: the pair $\{15,16\}$ admits
3-puncture escapes at every dyadic scale tested (e174, notes/74).  All
uses below consume only (H1$'$).
\end{remark}
...
\noindent\emph{Pair-extraction step (patched; notes/74 \S I.4): choose a
clean scale $m_0$ with $2^{m_0} \ge \max\{12C_0+25,\ x_0(C_0)\}$.  By
Lemma~\ref{lem:diagdense}, $B(m_0)$ contains $\ge (2^{m_0}-13)/12 \ge
C_0+1 > C_0$ pairwise-disjoint diagonal usable pairs; the $\le C_0$ dust
values puncture at most $C_0$ of them, so some pair
$\{3p, 3p+1\} \subseteq T$ survives the dust, and $3p > 2^{m_0} \ge
x_0(C_0)$ puts it inside (H1$'$)'s range.  Steps 2--3 of notes/52 \S3.1
then run verbatim with (H1$'$) in place of H1.}
...
\status{PROVED} \emph{(Proof: DEGS77, applied after restricting to
the progression and transporting by its affine parametrization.  When the
common difference is a power of two, HSPLIT gives an alternative proof.)}
\end{corollary}
...
shapes are closed by \S\ref{sec:chart}; Corollary~\ref{cor:hsplit}
imposes hereditary 2-adic mixing, and combined with
Corollary~\ref{cor:notail} a valid pair cannot be globally periodic ---
but hereditary 2-adic splitness alone does
...
Lemma~\ref{lem:pin} + Theorem~\ref{thm:b1}, whose hypothesis (H1$'$)
follows from Theorem~\ref{thm:c3p} at $C = 0$ (where $x_0(0) = 11$
suffices) plus Hypothesis~\ref{hyp:n3grow} at $C > 0$ with
$x_0(C) = \max\{11, 4C+5\}$, so that $|D| \le C < \lfloor (3p-1)/4
\rfloor$ for every pair the patched extraction step of
Theorem~\ref{thm:b1} produces.  In Case 2, Hypothesis~\ref{hyp:n6a}
closes the coupled core at
