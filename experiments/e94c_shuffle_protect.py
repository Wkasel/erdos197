"""Randomized-order protected deletion: does candidate t survive into a MUS
under any of NSH shuffled deletion orders? Args: M i1 i2 ..."""
...
for w in which:
    ...
    if t is None:
        print(f"M={M} pat{w}: NOT INSTANTIABLE", flush=True); continue
    ok = False
    for s in range(NSH):
        work = list(trips); rng.shuffle(work)
        r, core = solve_with(work); assert r is False
        work = [u for u in work if u in core or u == t]
        i = 0
        while i < len(work):
            if work[i] == t: i += 1; continue
            cand2 = work[:i] + work[i + 1:]
            r, c2 = solve_with(cand2)
            if not r: work = [u for u in cand2 if u in (c2 or cand2) or u == t]
            else: i += 1
        r, _ = solve_with([u for u in work if u != t])
        if r:
            print(f"M={M} pat{w} {pat} -> {t}: IN-SOME-MUS (shuffle {s})",
                  flush=True)
            ok = True; break
    if not ok:
        print(f"M={M} pat{w} {pat} -> {t}: NOT-CONFIRMED after {NSH} shuffles",
              flush=True)
