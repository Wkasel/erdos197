"""Erdős #197 — e165b: HEROIC coupled 3-block (2,2,2) core, lazy transitivity.
...
Usage: e165b_coupled_lazy.py M [--support core] [--bounds 2,2,2]
...
def main():
    ...
    for bi, blk in enumerate((B0, B1, B2)):
        ...
        for sign in (1, -1):
            ...
            for c in enc.clauses:
                sol.add_clause(c)
                ncard += 1
    print(f"e165b M={M} support={args.support} bounds={bounds} n={n} "
    ...
    while True:
        ...
        el = time.time() - t0
        if not ok:
            print(f"VERDICT e165b M={M} {args.support} {bounds}: UNSAT "
        ...
        if totv == 0:
            ...
            fn = f"data/e165b_M{M}_{args.support}{tagf}_witness.json"
            json.dump(info, open(fn, "w"))
            print(f"VERDICT e165b M={M} {args.support} {bounds}: SAT "
