"""e189_cms_lazy: e166_block_lazy with a cryptominisat (pycryptosat) backend
(notes/87 solver-engineering sprint).  Identical clause generation and lazy
transitivity CEGAR loop; only the solver API differs.  Original header:
...
def main():
    ...
    added = 0
    while True:
        ok, cmsmodel = sol.solve()
        el = time.time() - t0
        if not ok:
            print(f"VERDICT e189cms M={M} {args.attacks}: UNSAT "
                  f"({el:.0f}s, {rounds} rounds, {added} lazy clauses)",
                  flush=True)
            return
        truth = np.array(cmsmodel[1:nP + 1], dtype=bool)
        ...
        if nv == 0:
            ...
            fn = f"data/e189cms_{args.attacks}_M{M}{tagf}_witness.json"
            json.dump(order, open(fn, "w"))
            print(f"VERDICT e189cms M={M} {args.attacks}: SAT ({el:.0f}s, "
