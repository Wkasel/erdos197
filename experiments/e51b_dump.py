...
def ladder_law(X, d, max_rounds=10000):
    ...
    while True:
        ...
        for i in range(k):
            ...
            if added > 30000: break
        if not added:
            # also return per-level placed sets
            lvl = [sorted(v for v in V if P[i][v] in ms) for i in range(d)]
            return order, lvl
...
if __name__ == "__main__":
    ...
    if r is not False:
        json.dump({'order': r[0], 'levels': r[1]}, open(f'data/law_{X}_{d}_witness.json', 'w'))
